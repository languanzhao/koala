<template>
	<view class="leaveSchool">
		<view class="schoolCard">
			<view class="schoolInfo">
				<image :src="schoolInfo.avatar"></image>
				<view class="contain">
					<view class="top-name">
					...
					<view class="bottom-info">
						...
						<view>{{collegeLevel[schoolInfo.college_level]}}</view>
						<view>{{ property[schoolInfo.property] }}</view>
						<view class="iconfont icon-location-active"></view>
						<view>{{ schoolInfo.province }}</view>
						<view>{{schoolInfo.city}}</view>
						<view>{{schoolInfo.district}}</view>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="bottom">
			<view class="attention">注意：如若当前退款申请日期，已超出院校的报名截止日期，申请退学的用户，须移步到该院校招生办进行办理。</view>
			<view class="button">
		...
		<uni-popup ref='uniPopupRefund'>
			<view class="uniPopup-toast">
				...
				<view class="uniPopup-summary">取消报名后，已锁定学位将重新开放报名，若该专业名额招收完毕，将无法再报名！是否确定取消报名？</view>
				<view class="uniPopup-buttons">
...
<script>
import schoolCard from '@/components/school-card/index';
