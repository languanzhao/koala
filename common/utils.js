...
export function app_share_wx(type,schoolItem) {
	...
	let shareUrl = context.$projectUrl + 'h6/#/sub_school/pages/detail/index?id=' + schoolItem.id
	if(koala && koala.code){
...
//微信小程序登录
export function login_mp_wx(paramsCode){
	const that = context
	if(store.state.userInfo.id){
		//用户登录了
		that.$http({
...
export function login_wx_h5(type,paramsCode){
			const that = context
